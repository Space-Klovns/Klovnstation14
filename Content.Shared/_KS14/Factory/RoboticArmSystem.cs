...
using Content.Shared._KS14.Factory.Slots;
...
namespace Content.Shared._KS14.Factory;
...
public sealed class RoboticArmSystem : EntitySystem
{
    ...
    private void OnLinkAttempt(Entity<RoboticArmComponent> ent, ref LinkAttemptEvent args)
    {
        ...
        {
            // only 1 machine
            if (GetOutputMachine(ent.Comp) != null)
            {
                args.Cancel();
                return;
            }
        ...
        {
            // only 1 machine
            if (GetInputMachine(ent.Comp) != null)
            {
                args.Cancel();
                return;
            }
    ...
    {
        if (GetOutputMachine(ent.Comp) is {} machine && ent.Comp.OutputSlot is {} slot)
    ...
    {
        if (GetInputMachine(ent.Comp) is {} machine && ent.Comp.InputSlot is {} slot)
    ...
    {
        if (GetInputMachine(ent.Comp) is {} input && ent.Comp.InputMachinePort is {} inPort)
            ent.Comp.InputSlot = _automation.GetSlot(input, inPort, input: false);
        if (GetOutputMachine(ent.Comp) is {} output && ent.Comp.OutputMachinePort is {} outPort)
