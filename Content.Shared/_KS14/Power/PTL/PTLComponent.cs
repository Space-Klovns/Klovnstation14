...
public sealed partial class PTLComponent : Component
{
    ...
    [DataField] public double SpesosMultiplier = 1.0;

    [DataField] public double MinShootPower = 1e6; // 1 MJ

    [DataField, AutoNetworkedField] public float ShootDelay = 10f;
    [DataField, AutoNetworkedField] public Vector2 ShootDelayThreshold = new Vector2(2f, 10f); // X is min, Y is max
    [DataField, AutoNetworkedField] public bool ReversedFiring = false;

    [DataField] public Vector2 ShootOffset = new(0, -1);
    
    [ViewVariables] public TimeSpan RadiationResetAt = TimeSpan.Zero;
    [ViewVariables(VVAccess.ReadOnly)] public TimeSpan NextShotAt = TimeSpan.Zero;

    [DataField] public DamageSpecifier BaseBeamDamage = new();
    [DataField] public float DamageMultiplier = 2.0f;
