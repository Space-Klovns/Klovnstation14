...
public sealed class PipeNodeTeleporterTest : GameTest
{
    ...
    /// </summary>
    private async Task<(EntityUid Beacon, EntityUid Recipient, EntityUid Input, EntityUid Output)> SpawnSetup()
    {
        var pair = Pair;
        var server = pair.Server;
        ...
        await server.WaitAssertion(() =>
        {
            ...
            for (var x = 0; x <= 5; ++x)
            {
                for (var y = 0; y <= 5; ++y)
                {
                    mapSys.SetTile(grid, grid, new Vector2i(x, y), new Tile(1));
                }
            }

            // Beacon end: the input feeds the beacon from the tile to its south.
            input = entityMan.SpawnEntity(InputProto, new EntityCoordinates(grid, 0.5f, 0.5f));
            beacon = entityMan.SpawnEntity(BeaconProto, new EntityCoordinates(grid, 0.5f, 1.5f));

            // Recipient end, five tiles away: the output pulls from the recipient on the tile to its north.
            output = entityMan.SpawnEntity(OutputProto, new EntityCoordinates(grid, 4.5f, 3.5f));
            recipient = entityMan.SpawnEntity(RecipientProto, new EntityCoordinates(grid, 4.5f, 4.5f));
        });
